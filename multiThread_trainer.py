...
class Trainer:
    ...
    def get_model(self):
        ...
        if model_name == 'LCNN':
            ...
            model = CNNLSTM(32,128,2,3).to(self.device)
        else:
            raise Exception(f"Error: {model_name}")
        return model

    def load_data(self):
        test_data_x, test_data_y = utils.load_data_npz(self.hps)
    ...
    def run(self):
        ...
        for epoch in range(self.hps.train.epoch):
            ...
            train_data_gen = utils.data_generator(train_file_all,self.hps.data.load_data_batch)  
